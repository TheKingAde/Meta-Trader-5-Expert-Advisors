...
// Simple moving averages
   sma_handle1M = iMA(Symbol(), PERIOD_CURRENT, SMA_Period, 0, MODE_SMA, PRICE_CLOSE);
   ...
   if(signalCreated == 1 && WFC == 0)
     {
      if(levelPrices[1] > levelPrices[2]) // First retracement after signal has been generated
         MonitorPrice(last_tick.bid, true);
     }
   else
      if(signalCreated == -1 && WFC == 0)
        {
         if(levelPrices[2] > levelPrices[1]) // First retracement after signal has been generated
            MonitorPrice(last_tick.ask, false);
        }

   if(WFC == 1)
     {
      double lastCandleStickClose = iClose(Symbol(), PERIOD_CURRENT, 1);

      if(lastCandleStickClose >= current1MSMA && last_tick.bid > levelPrices[2])
        {
         double stopLoss = calculateSL(last_tick.bid, true);
         double slPoints = MathAbs(last_tick.bid - stopLoss);
         double tpPoints = slPoints * 4;
         double takeProfit = last_tick.bid + tpPoints;
      ...
      if(WFC == -1)
        {
         ...
         if(lastCandleStickClose <= current1MSMA && last_tick.ask < levelPrices[2])
           {
            double stopLoss = calculateSL(last_tick.ask, false);
            double slPoints = MathAbs(last_tick.ask - stopLoss);
            double tpPoints = slPoints * 4;
            double takeProfit = last_tick.ask - tpPoints;
            int orderStatus = 0;
...
//+------------------------------------------------------------------+
void MonitorPrice(double lastTickPrice, bool isBullish)
  {
   if(isBullish)
     {
